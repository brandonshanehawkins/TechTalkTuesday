...
- [x] Create a Note Node that can display a larger, wrapped text field; for explanations or scratchpad thoughts, etc.
- [x] Create a new custom node that has the TTT brand colors coded in, or even better, TTT brand colors by defualt, but the fields are editable for use with other color themes. Each color should have an output wire.
- [x] I like 'Combine Elements' node you added... I want an 'element' node intended for the 'Combine Elements' (HTML Container node added with justification options).
- [x] I'm not sure if the save image node is working like I had hoped. Is there a node that needs to go between to make 'pixels' for the save image node? (Added Render To Image Node).
